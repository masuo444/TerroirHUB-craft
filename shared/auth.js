// Terroir HUB — Unified Supabase Auth Integration
// 全ジャンル共通。window.THUB_CONFIG で分岐。
...
(function(){
  ...
  var BRAND_COLOR_2 = CFG.brandColor2 || '#D4728A';
  var BASE_PATH = CFG.basePath || '/sake';
  var SITE_NAME = CFG.siteName || 'Terroir HUB SAKE';
  var API_BASE = CFG.apiBase || '';  // empty = same origin, otherwise 'https://sake.terroirhub.com'
  ...
  function initAuth() {
    ...
    // ══════════════════════════════════════
    // Session migration: localStorage → cookie
    // ══════════════════════════════════════
    var OLD_KEY = 'sb-hhwavxavuqqfiehrogwv-auth-token';
    if (window.ThubCookieStorage) {
      var oldSession = localStorage.getItem(OLD_KEY);
      if (oldSession && !window.ThubCookieStorage.getItem('sb-thub-auth')) {
        window.ThubCookieStorage.setItem('sb-thub-auth', oldSession);
        localStorage.removeItem(OLD_KEY);
        console.log('[AUTH] Migrated session from localStorage to cookie');
      }
    }
    ...
    const sb = createClient(SUPABASE_URL, SUPABASE_KEY, {
      auth: {
        storage: window.ThubCookieStorage || localStorage,
        storageKey: 'sb-thub-auth',
        flowType: 'implicit',
      }
    });
    ...
    function onLogin(user) {
      ...
      if (authBtns) {
        authBtns.innerHTML = '<button onclick="thubShowMypage()" style="display:flex;align-items:center;gap:10px;background:linear-gradient(135deg,' + BRAND_COLOR + ',' + BRAND_COLOR_2 + ');border:none;border-radius:28px;padding:6px 18px 6px 6px;cursor:pointer;transition:all 0.15s;box-shadow:0 3px 12px rgba(184,69,42,0.25);" onmouseover="this.style.transform=\'scale(1.03)\'" onmouseout="this.style.transform=\'scale(1)\'">' +
      ...
      // DB-backed credits display
      sb.rpc('get_credits', { p_user_id: user.id }).then(function(res) {
    ...
    function onLogout() {
      ...
      const authBtns = document.querySelector('.nav-auth');
      if (authBtns) {
        authBtns.innerHTML = '<button class="nav-login-btn" onclick="showAuth(\'login\')">ログイン</button>' +
    ...
    window.thubShowMypage = function(tab) {
      ...
      var planColor = plan === 'premium' ? 'linear-gradient(135deg,' + BRAND_COLOR + ',#8B3520)' : plan === 'pro' ? 'linear-gradient(135deg,' + BRAND_COLOR_2 + ',' + BRAND_COLOR + ')' : '#eee';
      var planTextColor = plan === 'free' ? '#999' : '#fff';

      // タブコンテンツ生成
      function buildHome() {
        var upgradeBtn = '';
        if (plan === 'free') upgradeBtn = '<button onclick="this.closest(\'#mypage-modal\').remove();if(typeof thubSubscribe===\'function\')thubSubscribe(\'pro\')" style="width:100%;padding:11px;background:' + BRAND_COLOR + ';color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer;margin-top:12px;">Proにアップグレード</button>';

        var creditHtml = '<div style="display:flex;justify-content:space-between;align-items:center;padding:12px 0;border-bottom:1px solid #f0f0f0;"><span style="font-size:13px;color:#666;">クレジット残高</span><div><span style="font-size:20px;font-weight:600;color:' + BRAND_COLOR + ';">' + credits + '</span></div></div>' +
          '<div style="font-size:11px;color:#999;margin-top:8px;">1クレジット = 120文字まで。121-240文字は2クレジット、241-300文字は3クレジットです。</div>';

        var logs = JSON.parse(localStorage.getItem('thub_sake_log') || '[]');

        var summary = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin:16px 0;">' +
          '<div style="text-align:center;background:#fafaf8;padding:12px 4px;border-radius:10px;"><div style="font-size:22px;font-weight:700;color:' + BRAND_COLOR + ';">' + logs.length + '</div><div style="font-size:10px;color:#aaa;">飲酒記録</div></div>' +
          '<div style="text-align:center;background:#fafaf8;padding:12px 4px;border-radius:10px;"><div style="font-size:22px;font-weight:700;color:' + BRAND_COLOR + ';">' + favs.length + '</div><div style="font-size:10px;color:#aaa;">お気に入り</div></div>' +
          '<div style="text-align:center;background:#fafaf8;padding:12px 4px;border-radius:10px;"><div style="font-size:22px;font-weight:700;color:' + BRAND_COLOR + ';">' + stamps.length + '</div><div style="font-size:10px;color:#aaa;">チェックイン</div></div>' +
        '</div>';
        ...
        setTimeout(function(){
          ...
          navigator.geolocation.getCurrentPosition(function(pos){
            ...
            fetch(BASE_PATH + '/search_index.json').then(function(r){return r.json()}).then(function(data){
              // 各県のざっくり緯度経度（中心点）
              var PREF_COORDS = {
              ...
              el.innerHTML = nearby.map(function(b){
                return '<a href="' + BASE_PATH + '/'+b.p+'/'+b.id+'.html" onclick="this.closest(\'#mypage-modal\').remove()" style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f5f5f5;text-decoration:none;color:inherit;">' +
                  '<div style="width:36px;height:36px;background:linear-gradient(135deg,' + BRAND_COLOR_2 + ',' + BRAND_COLOR + ');border-radius:50%;display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:600;flex-shrink:0;">'+(b.n?b.n[0]:'酒')+'</div>' +
                  '<div style="flex:1;min-width:0;"><div style="font-size:13px;font-weight:500;color:#333;">'+escHtml(b.n||'')+'</div><div style="font-size:11px;color:' + BRAND_COLOR + ';">'+escHtml(b.b||'')+'</div><div style="font-size:10px;color:#aaa;">'+(PN[b.p]||b.p)+' '+escHtml(b.a||'')+'</div></div>' +
                  '<div style="font-size:10px;color:' + BRAND_COLOR + ';flex-shrink:0;">チェックイン →</div></a>';
              }).join('');
            });
          }, function(){
            var el = document.getElementById('nearby-list');
        ...
        if (isLineUser) {
          ...
          var lineLoginUrl = API_BASE + '/api/line-login' + (currentUser ? '?user_id=' + currentUser.id : '');
      ...
      function buildHistory() {
        ...
        return history.slice(0, 20).map(function(h) {
          ...
          return '<a href="' + BASE_PATH + '/' + encodeURIComponent(h.pref) + '/' + encodeURIComponent(h.id) + '.html" onclick="this.closest(\'#mypage-modal\').remove()" style="display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid #f5f5f5;text-decoration:none;color:inherit;">' +
      ...
      function buildTaste() {
        ...
        return '<div style="font-size:13px;color:#666;margin-bottom:16px;">好みを設定すると、サクラの提案が合いやすくなります。</div>' +
          ...
          '<div style="margin-bottom:16px;">' +
            ...
            '</div>' +
          '</div>' +
          '<button onclick="saveTasteFromUI()" style="width:100%;padding:11px;background:' + BRAND_COLOR + ';color:#fff;border:none;border-radius:8px;font-size:13px;cursor:pointer;">保存する</button>';
      }
      ...
      function buildTempChip(val, current, label) {
        ...
        return '<button onclick="document.getElementById(\'taste-temp\').value=\'' + val + '\';this.parentElement.querySelectorAll(\'button\').forEach(function(b){b.style.background=\'#fff\';b.style.color=\'#666\';b.style.borderColor=\'#eee\';});this.style.background=\'' + BRAND_COLOR + '\';this.style.color=\'#fff\';this.style.borderColor=\'' + BRAND_COLOR + '\';" style="font-size:11px;padding:6px 12px;border-radius:16px;border:1px solid ' + (isActive ? BRAND_COLOR : '#eee') + ';background:' + (isActive ? BRAND_COLOR : '#fff') + ';color:' + (isActive ? '#fff' : '#666') + ';cursor:pointer;">' + (label || val) + '</button>';
      }
        ...
        if (logs.length === 0) {
          ...
          html += logs.slice(0, 10).map(function(l){
            ...
            return '<div style="padding:8px 0;border-bottom:1px solid #f5f5f5;">' +
              ...
              (l.brand ? '<div style="font-size:12px;color:' + BRAND_COLOR + ';">' + escHtml(l.brand) + '</div>' : '') +
              (l.rating > 0 ? '<div style="color:' + BRAND_COLOR + ';font-size:12px;">' + stars + '</div>' : '') +
            '</div>';
          }).join('');
        }
        ...
        if (wishlist.length === 0) {
          ...
          html += wishlist.map(function(w){
            return '<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid #f5f5f5;">' +
              ...
              (w.brand ? '<div style="font-size:11px;color:' + BRAND_COLOR + ';">' + escHtml(w.brand) + '</div>' : '') + '</div></div>';
          }).join('');
        }
        ...
        return '<div style="font-size:12px;color:#888;margin-bottom:8px;">' + list.length + ' 銘柄</div>' +
          list.map(function(w){
            return '<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid #f5f5f5;">' +
              ...
              (w.brand ? '<div style="font-size:12px;color:' + BRAND_COLOR + ';">' + escHtml(w.brand) + '</div>' : '') + '</div>' +
      ...
      function buildStamps() {
        ...
        var header = '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:16px;">' +
          '<div style="text-align:center;background:#fafaf8;padding:12px 8px;border-radius:10px;"><div style="font-size:20px;font-weight:700;color:' + BRAND_COLOR + ';">' + stamps.length + '</div><div style="font-size:10px;color:#aaa;">蔵</div></div>' +
          '<div style="text-align:center;background:#fafaf8;padding:12px 8px;border-radius:10px;"><div style="font-size:20px;font-weight:700;color:' + BRAND_COLOR + ';">' + prefSet.size + '/47</div><div style="font-size:10px;color:#aaa;">都道府県</div></div>' +
        '</div>';
      ...
      function buildQuest(){
        ...
        if(quests.length === 0){
          return '<div style="text-align:center;padding:32px 0;">' +
            ...
            '<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:8px;margin-bottom:20px;text-align:center;">' +
              '<div style="background:#fdf5f6;border-radius:10px;padding:12px;"><div style="font-size:24px;font-weight:700;color:' + BRAND_COLOR + ';">0</div><div style="font-size:10px;color:#aaa;">投稿数</div></div>' +
              '<div style="background:#fdf5f6;border-radius:10px;padding:12px;"><div style="font-size:24px;font-weight:700;color:' + BRAND_COLOR + ';">📸</div><div style="font-size:10px;color:#aaa;">初投稿で<br>バッジ獲得</div></div>' +
            '</div>' +
        ...
        var html = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:16px;text-align:center;">' +
          '<div style="background:#fdf5f6;border-radius:10px;padding:10px;"><div style="font-size:22px;font-weight:700;color:' + BRAND_COLOR + ';">' + quests.length + '</div><div style="font-size:9px;color:#aaa;">投稿数</div></div>' +
          '<div style="background:#fdf5f6;border-radius:10px;padding:10px;"><div style="font-size:22px;font-weight:700;color:' + BRAND_COLOR + ';">' + new Set(quests.map(function(q){return q.brewery_id})).size + '</div><div style="font-size:9px;color:#aaa;">蔵元数</div></div>' +
          '<div style="background:#fdf5f6;border-radius:10px;padding:10px;"><div style="font-size:22px;font-weight:700;color:' + BRAND_COLOR + ';">' + (quests.length >= 30 ? '👑' : quests.length >= 10 ? '🏆' : quests.length >= 5 ? '🔍' : '📸') + '</div><div style="font-size:9px;color:#aaa;">ランク</div></div>' +
        '</div>';
      ...
      var tabBar = '<div style="display:flex;border-bottom:1px solid #f0f0f0;margin:-4px -28px 16px;padding:0 28px;">' +
        tabs.map(function(t) {
          ...
          return '<button onclick="this.closest(\'#mypage-modal\').remove();thubShowMypage(\'' + t.id + '\')" style="flex:1;padding:10px 0;font-size:11px;border:none;background:none;cursor:pointer;color:' + (isActive ? BRAND_COLOR : '#aaa') + ';border-bottom:2px solid ' + (isActive ? BRAND_COLOR : 'transparent') + ';font-weight:' + (isActive ? '600' : '400') + ';font-family:Noto Sans JP,sans-serif;">' + t.icon + '<br>' + t.label + '</button>';
        }).join('') + '</div>';
      ...
      modal.innerHTML = '<div style="background:#fff;border-radius:16px;max-width:440px;width:calc(100% - 32px);padding:28px;box-shadow:0 16px 48px rgba(0,0,0,0.12);max-height:85vh;min-height:70vh;overflow-y:auto;">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">' +
          '<div style="display:flex;align-items:center;gap:10px;">' +
            '<div style="width:40px;height:40px;background:linear-gradient(135deg,' + BRAND_COLOR_2 + ',' + BRAND_COLOR + ');border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;color:#fff;font-weight:600;">' + escHtml(nickname ? nickname[0].toUpperCase() : email[0].toUpperCase()) + '</div>' +
            '<div>' +
              '<div style="display:flex;align-items:center;gap:6px;">' +
                '<span id="mypage-nickname" style="font-size:14px;font-weight:500;color:#333;">' + escHtml(nickname || email.split('@')[0]) + '</span>' +
                '<button onclick="thubEditNickname()" style="background:none;border:none;color:' + BRAND_COLOR + ';font-size:11px;cursor:pointer;">✏️</button>' +
              '</div>' +
    ...
    function updatePlanBadge() {
      ...
      if (currentPlan === 'premium') {
        ...
        badge.style.cssText = 'font-size:10px;background:' + BRAND_COLOR + ';color:#fff;padding:2px 6px;border-radius:4px;margin-left:6px;';
      } else if (currentPlan === 'pro') {
        badge.textContent = 'Pro';
        badge.style.cssText = 'font-size:10px;background:' + BRAND_COLOR_2 + ';color:#fff;padding:2px 6px;border-radius:4px;margin-left:6px;';
      } else {
        badge.textContent = '';
    ...
    window.thubSubscribe = async function(plan) {
      ...
      try {
        ...
        if (!accessToken) throw new Error('再ログインしてください');
        ...
        var res = await fetch(API_BASE + '/api/create-subscription', {
          method: 'POST',
          headers: {
    ...
    function checkPlanActivation() {
      ...
      if (activated) {
        ...
        if (window.thub && typeof window.thub.purchase === 'function') {
          window.thub.purchase(activated, activated === 'premium' ? 1500 : 500);
        }
