...
(function(){
  ...
  // Extract brewery info from URL: /sake/{pref}/{id}.html
  ...
  function sendToSupabase(payload){
    ...
    fetch(SB_URL + '/rest/v1/analytics', {
      ...
      if(!res.ok){
        throw new Error('analytics write failed: ' + res.status);
      }
    }).catch(function(err){
      console.warn('[THUB] analytics send failed', err && err.message ? err.message : err);
  ...
  function flushQueue(){
    try {
      ...
      fetch(SB_URL + '/rest/v1/analytics', {
        ...
        if(q.length > 0) setTimeout(flushQueue, 1000);
      }).catch(function(err){
        console.warn('[THUB] analytics flush failed', err && err.message ? err.message : err);
        const rollback = JSON.parse(localStorage.getItem(QUEUE_KEY) || '[]');
        localStorage.setItem(QUEUE_KEY, JSON.stringify(batch.concat(rollback)));
      });
    } catch(e){}
  }
